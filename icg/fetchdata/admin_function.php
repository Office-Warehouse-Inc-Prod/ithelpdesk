...
class dbconfig extends dbconn
{
	...
	public function bargrph_tech_res()
	{
		...
		$query = "SELECT
				...
				WHERE
				reports.sub_id NOT IN (15,28,34,35) AND reports.itsup IN (1,2,5,7,10,11,14,44,45,47,77) AND reports.f_deptsel = '12' AND
...
public function deptthist() {
	...
	public function linegraph()
	{
		...
		reports
		WHERE
		year(date_created) BETWEEN '2019' AND '2025' and reports.sub_id NOT IN ('15','28','34','35') AND reports.f_deptsel = '12'
		GROUP BY
		DATEPART";
		}
...
";
	...
	public function admin_data_table_res()
	{
		...
		WHERE (
			(vw6.deptsel = '12' AND vw6.status NOT IN ('NEW REPORT', 'Assigned', 'ASSIGNED'))
			OR 
			(users.deptsel = '12' AND vw6.status NOT IN ('NEW REPORT'))
		)
	...
	public function admin_data_table_transfer()
{
    ...
    WHERE vw_transfer.deptsel = '12' 
    AND vw_transfer.f_deptsel NOT IN ('12') 
...
";
	...
	public function dtbl_itsup()
	{
		...
		$query = "
	Select * from vw6 WHERE
	vw6.sub_id NOT IN ('15','28','34','35') AND status <> 'NEW REPORT' AND itsup = " . $_POST['itVal'] . " AND years IN (" . $_POST['yrsx1'] . ") AND deptsel = 12 ORDER BY `status` DESC";
		$statement = $this->connection->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
		$data[] = array();
