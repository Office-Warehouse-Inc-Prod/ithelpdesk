...
<div class="card-title">WAREHOUSE PULL OUT: <span class="" id="count_sched" style="font-size: 18px;"></span></div>
                          
</div>                                  
<div class="card-footer d-flex align-items-center justify-content-between">

                     <a class=" second small text-white stretched-link" id="card_sched" href="#bottom" value="WAREHOUSE PULL OUT" >Click here for more info.</a>
...
<div class="card-title">READY FOR PULL OUT: <span class="float-right" id="count_pickup" style="font-size: 32px;"></span></div>
</div>
<div class="card-footer d-flex align-items-center justify-content-between">

                       <a class=" text-white stretched-link" id="card_ready" href="#bottom" value="READY FOR PULL OUT" ><span class="small text-white">Click here for more info.</span></a>
...
<div class="card-title">CONFIRM PULL OUT: <span class="float-right" id="count_confirm" style="font-size: 32px;"></span></div>

</div>
<div class="card-footer d-flex align-items-center justify-content-between">

                       <a class=" text-white stretched-link" id="card_confirm" href="#bottom" value="CONFIRM PULL OUT" ><span class="small text-white">Click here for more info.</span></a>
...
<label id="tripticket1lbl">TRIP TICKET# (FOR PULL OUT)</label>
<input type="text" class="form form-control" name="tripticket1" id="tripticket1" placeholder="READY TO PULL OUT">
</div>
...
<div class="modal fade" id="Pd_Items" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="Pd_ItemsLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    ...
    <div class="col-md-4">
    <label>STATUS</label>
    <input type="text" class="form form-control nstatus" name="new_status" id="new_status" readonly>
<select class = "form form-control ostatus" name= "newstatus" id="newstatus">
<option value=""> &larr; Status &rarr;</option>
<?php
      $query="SELECT
	`status`.stat_id, 
	`status`.stat_desc, 
	`status`.pd_module_tag_import, 
	`status`.pd_import_seq
FROM
	`status`
WHERE
pd_module_tag_import = 'Y'
ORDER BY pd_import_seq ASC";
      $run=$conn->prepare($query);
      $run->execute();
      $rs=$run->get_result();
      while ($res=$rs->fetch_assoc()) {
...
$("#status").change(function (e) { 
  ...
  if (Multitag != 'Y') {
      ...
      $('#tripticket1').attr('readonly', false);
      }
      else if (Status == "READY FOR PULL OUT") {
      $('.divhide').css({'display':'block'});
      $('.divdesc').css({'display':'block'});
      $('.divserial').css({'display':'block'});
      ...
      $('.divrtv').css({'display':'none'});
      $('.divcm').css({'display':'none'});
      $('#tripticket1').attr('readonly', true);
      }
      else{
      $('.divhide').css({'display':'block'});
      $('.divdesc').css({'display':'block'});
  ...
  if (Status == "RETURN TO STORE") {
      ...
      else if (Status == "WAREHOUSE PULL OUT") {
      $('.divhide').css({'display':'block'});
      $('.divdesc').css({'display':'block'});
      $('.divserial').css({'display':'block'});
      $('.trip1').css({'display':'none'});
...
if (user_id == '260') {
    table.search('FURNITURE').draw();
   }

$('#report_data tbody').on( 'click', 'button', function () {
var data = table.row( $(this).parents('tr') ).data();
$('#subjct').attr('readonly', true);
$('#itsup, #store, #sub, #cat, #date_created, #via').val(function() {
...
// var val =  $(this).attr("value");
var value = "WAREHOUSE PULL OUT|READY FOR PULL OUT|RETURN TO STORE|APRROVED|EVLAUATE|REPAIRED|SCHEDULE FOR DISPOSAL|SUBJECT FOR ADJUSTMENT|APPROVED SUMMARY ADJUSTMENT|OKAY FOR PULL OUT|CONFIRM PULL OUT|ITEM RECEIVED|LIST FOR DISPOSAL|SUBJECT FOR CLOSING";
// alert(value);
table
.columns( 7 )
...
{title:"STATUS", data:"n_status","defaultContent": "",},
{title:"", data:"statusx", "width": "5%","defaultContent": "",},
{
        title: "ACTION",
        data: null,
