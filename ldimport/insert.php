...
$data=   array(
  ...
  $statement = $connection->prepare(
   "UPDATE reports
   SET ticket_no = :ticket_no, $brid date_created = :date_created,  via = :via, control_no = :control_no,
                    status = :status, $itsup $cat_id $sub_id $ispid refNo = :refNo, date_refNo = :date_refNo, date_closed = :date_closed, $clby remarks = :remarks
   WHERE ticket_no = :ticket_no"
  );
...
if (isset($_POST['operation3']) && $_POST['operation3'] == 'Update') {
  ...
  $statement = $connection->prepare("UPDATE tbl_pditems SET  n_status = :n_status, item_status = :itemstat WHERE id = '$IDx'");
