...
function _storesgraph(s_area,syr){
                          $.ajax({
                  url:"fetchdata/fetch_data.php",
                  method:'POST',
                   data:{area_desc:s_area,yr:syr,mode:'strs_grph'},

                  success:function(fdata)
                  {
                    var objstorearea = JSON.parse(fdata);
                    _plot_stores_graph(objstorearea);
                    $('#stores_graph_modal').modal({"show": true, "backdrop": 'static'});
                  }
                 });
}
...
function _plot_stores_graph(strdata){
...
// Create chart instance
var chart = am4core.create("stores_graph", am4charts.XYChart);
