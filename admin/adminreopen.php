...
<head>
    ...
    <style>
        ...
        #re_open_table th, #re_open_table td {
        ...
        #re_open_table tbody tr:hover td { 
        ...
        #reopen_logs_table th, #reopen_logs_table td {
            border-left: none !important;
            border-right: none !important;
            border-top: none !important;
            border-bottom: 1px solid #e5e7eb !important; 
        ...
        #reopen_logs_table tbody tr:hover td { 
            background-color: #d2d9e6 !important;
            color: black !important; 
            cursor: pointer; 
            transition: background-color 0.2s ease; 
...
<div class="container mt-4">
    <div class="mb-3">
            <button type="button" id="strbtnact" class="btn mb-3" data-toggle="modal" data-target="#reopen_logs_modal">
      <i class="fas fa-ticket"></i>&nbsp; REOPENED TICKET LOGS
    </button>
       
    </div>
    <div class="table-responsive">
        <table class="table table-hover" id="re_open_table"></table>
    </div>
</div>

<script src="../js/coms.js"></script> 
<div class="modal fade" id="reopen_logs_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
 <div class="modal-dialog" style="max-width: 96%; width: 96%;"> 
    <div class="modal-content">
      <div class="modal-header"  style="background-color: #213456; color: white; border-bottom: 3px solid #E1AD01;">
        <h5 class="store-title" id="exampleModalLabel">
          <i class="fas fa-ticket"></i> REOPENED TICKET LOGS
        </h5>
      ...
      <div class="modal-body">
        <div class="table-responsive">
            <table class="table table-hover" id="reopen_logs_table" style="width:100%;"></table>
        </div>
      </div>
    </div> 
  </div>
...
<!-- Split Modal UI -->
<div class="col-12 col-lg-12 modal fade" id="reopen_Modal" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" style="max-width: 96%; width: 96%;"> 
        <form method="post" id="reopen_form" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header" style="background-color: #213456; color: white; border-bottom: 3px solid #E1AD01;">
...
<!-- 
                                ...
                                <div class="form-group col-md-12 mb-0">
                                    ...
                                    <textarea name="remarks" id="remarks" rows="2" class="form-control form-control-sm custom-select-placeholder placeholder-active" placeholder="Your Reason" style="height: 200px; text-transform:uppercase" required onchange="handleDropdownChange(this)"></textarea>
                                </div>
                                     <div class="form-group col-md-4">
                                        <label>STATUS</label>
                        ...
                        <div class="col-md-6 p-4" style="background: linear-gradient(to bottom, #f8f9fa, #d7dce4);">
                            <div class="d-flex flex-column h-100" id="msg_thread">
                                ...
                                <div class="mt-auto">
                                    ...
                                    <textarea name="admsg" class="form-control" rows="3" placeholder="Reply to their message or give an update regarding this ticket..." required></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
               ...
               <div class="modal-footer d-flex justify-content-end">
                    ...
                    <button type="submit" id="action" class="btn btn-success">REOPEN</button>
                </div>
            </div>
        </form>
    </div>
...
function loadDeptsel(itsup_value) {
    if (itsup_value && itsup_value !== "0") {
        ...
        $.ajax({
            ...
            success: function(response) { $('#deptsel').val(response.trim()); },
            error: function() { $('#deptsel').val("Error fetching data"); }
        });
    } else {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (Array.isArray(response) && response.length > 0) {
                const currentUserIdStr = "<?= $_SESSION['user_id'] ?? '' ?>";
                const currentUserNameStr = "<?= $_SESSION['fname'] ?? '' ?>";
                ...
                sortedResponse.forEach(function(comment, index) {
                    ...
                    let isMe = (currentUserIdStr !== "" && sender === currentUserIdStr) || 
                               (currentUserNameStr !== "" && sender.includes(currentUserNameStr));
...
$(document).ready(function() {
    ...
    function fetchMainTable() {
        $.post('fetchdata/fetch_data.php', {mode: 'reopen_tbl'}, function(data) {
            admin_datatable(data);
        }, 'json').fail(function(xhr) {
            console.error("Failed to load main table data:", xhr.responseText);
        });
    }

    function fetchLogsTable() {
        $.post('fetchdata/fetch_data.php', {mode: 'reopen_logs_table'}, function(data) {
            logs_datatable(data); 
        }, 'json').fail(function(xhr) {
            console.error("Failed to load logs table data:", xhr.responseText);
        });
    }
    ...
    const tableColumns = [
            ...
            render: function(data, type, row) {
                if ((type === 'sort' || type === 'type') && data) {
                    ...
                    if (parts.length > 1) {
                        ...
                        let time = parts[1];
                        if(date.length === 3) return `${date[2]}-${date[0]}-${date[1]} ${time}`;
                    }
                }
                return data; 
            }
        ...
        {title: "Action", data: null, defaultContent: "<Button class='btn btn-primary btn-sm' name='update' style='background:#213456; color:white; border:none;'><i class='fas fa-rotate-left'></i> REOPEN</Button>"}
    ];

const logColumns = [
    {title: "Ticket No", data: "ticket_no", defaultContent: ""},
    {title: "Reopened By", data: "reopened_by", defaultContent: ""},
    {title: "Reopened Date", data: "reopened_date", defaultContent: ""},
    {title: "Reason", data: "reason", defaultContent: ""}
];

    function admin_datatable(t) {
        const dataset = t.reopendata || t.data || [];
        reptable = $("#re_open_table").DataTable({
        ...
        $('#re_open_table tbody').off('click', 'button').on('click', 'button', function () {
            openReopenModal(reptable.row($(this).parents('tr')).data());
        });
    }

    function logs_datatable(t) {
        const dataset = t.logsdata || t.data || [];
        logstable = $("#reopen_logs_table").DataTable({
            "dom": '<"pull-left"lf><"pull-right">tip',
            "stateSave": false,
            "bDestroy": true,
            "responsive": true, 
            ...
            "autoWidth": false,
            "language": {
                "emptyTable": "No reopened logs found",
                "search": "_INPUT_",
        ...
        $('#reopen_logs_table_filter input').off('keyup').on('keyup', function () {
          logstable.search($(this).val()).draw();
        });

        $('#reopen_logs_table tbody').off('click', 'button').on('click', 'button', function () {
            openReopenModal(logstable.row($(this).parents('tr')).data());
        });
    }

    $('#reopen_logs_modal').on('shown.bs.modal', function () {
        if ($.fn.DataTable.isDataTable('#reopen_logs_table')) {
            $('#reopen_logs_table').DataTable().columns.adjust().responsive.recalc();
        }
    });
    ...
    $(document).on('change', '#f_deptsel', function () {
        ...
        if (dept_id !== '') {
            $.ajax({
                ...
                success: function (response) { $('#contactNumber').val(response); },
                error: function () { $('#contactNumber').val(''); }
            });
        } else {
            $('#contactNumber').val('');
        }
    ...
    $(document).on('submit', '#reopen_form', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                let isSuccess = (typeof response === 'object' && (response.status === 'success' || response.status === true)) ||
                ...
                if (isSuccess) {
                    Swal.fire({
                        ...
                        refreshAllData(); // Refresh both tables
                    });
                } else {
            ...
            error: function(xhr) {
                ...
                let errorMsg = xhr.responseText || 'Failed to process request.';
   ...
   function openReopenModal(data) {
        if(!data) return;
        $('#reopen_logs_modal').modal('hide');
        $('#tick_title').text("TICKET #: " + data['ticket_no']);
        $('#ticket_no').val(data['ticket_no']);
        $('#str_desc').val(data['str_code'] ? data['str_code'] : data['dept_desc']); 
        $('#crtd_by').val(data['full_name']);
        $('#date_createdx').val(data['date_created']);
        $('#concern').val(data['concern']); // Subject Input
