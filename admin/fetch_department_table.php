...
if (isset($_POST['yr']) && isset($_POST['mo'])) {
    ...
    CONCAT(TIMESTAMPDIFF(HOUR, req.created_at, res.date_rasigned), 'h ', MOD(TIMESTAMPDIFF(MINUTE, req.created_at, res.date_rasigned), 60), 'm ', MOD(TIMESTAMPDIFF(SECOND, req.created_at, res.date_rasigned), 60), 's') AS turnaround_time FROM tbl_reassigned res 
    ...
    WHERE YEAR(req.created_at) IN ($year_placeholders) AND MONTH(req.created_at) IN ($month_placeholders) ORDER BY res.date_rasigned DESC";
    ...
    $query_totals = "SELECT 
        ...
        if ($stmt = $db->prepare($query3)) { $stmt->bind_param($types, ...$params); $stmt->execute(); $res = $stmt->get_result()->fetch_all(MYSQLI_ASSOC); $stmt->close(); 
            $tot = array_sum(array_column($res, 'ticket_count'));
            foreach ($res as &$r) { $r['ticket_percentage'] = $tot > 0 ? round(($r['ticket_count'] / $tot) * 100, 2) : 0; }
