...
public function faprintingthist() {
        $query="SELECT 
            ...
            LEFT JOIN users u ON r.userId = u.id
			  LEFT JOIN it_tech itt ON ar.noted_by = itt.itsup
            LEFT JOIN tbl_branch b ON r.store = b.str_num  WHERE ar.status IN ('RECORDED')  ORDER BY ar.created_at ASC";
        $statement = $this->connection->prepare($query);
        $statement->execute();
        $result = $statement->fetchAll();
        $fetchdata = array();
...
public function farecordingthist() {
        $query="SELECT 
                ar.ticket_no, 
                b.str_name, 
                CONCAT(u.fname, ' ', u.lstname) AS full_name, 
...
public function fareportsthist() {
	...
	public function linegraph()
	{
		...
		reports
		WHERE
		year(date_created) BETWEEN '" . $_POST['yr'] . "' AND '" . $_POST['yr'] . "' and reports.sub_id NOT IN ('15','28','34','35') AND reports.f_deptsel = '2' AND cat_id IN ('37','38','39','40','41')
		GROUP BY
		DATEPART";
		} else {
			$query = "
	...
	public function pie()
	{
		...
		FROM vwp 
		WHERE deptsel = '2' AND cat_id IN ('37','38','39','40','41') AND date_created IN (" . $_POST['yr'] . ")  AND cat_desc <> 'GENERAL'
	...
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( " . $_POST['yr'] . " ) AND f_deptsel = '2' AND cat_id IN ('37','38','39','40','41')
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement->execute();
...
";
			...
			select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (" . $_POST['yr'] . ") AND area_desc = '" . $_POST['area_desc'] . "' AND f_deptsel = '2' group by reports.store, str_code, area_desc ORDER BY str_code ASC
