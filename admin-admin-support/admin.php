...
<body>
          ...
          <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="genReportDrop">
              ...
              <a class="dropdown-item" href="performance.php">
                <i class="fa-solid fa-arrow-trend-up"></i> Admin Performance
              </a>
            </div>
          </li>
          <!--<li class="nav-item dropdown">
  ...
  { 
    ...
    async function getValidation() {
      try {
        const response = await fetch("fetchdata/notif_validation.php?_=" + Date.now());
        const count = (await response.text()).trim();
        const badge = document.getElementById("notif_validation");
        if (!badge) return;
     ...
     async function getPrinting() {
      try {
        const response = await fetch("fetchdata/notif_printing.php?_=" + Date.now());
        const count = (await response.text()).trim();
        const badge = document.getElementById("notif_printing");
        if (!badge) return;
