...
var types = $.ajax({
 ...
 function grhp(types) {
  am4core.ready(function () {
    ...
    pieSeries.columns.template.events.on("hit", function(ev){
      let cat = ev.target.dataItem.dataContext["type"];

      _catgraph(cat, curyr2);
},this);
...
var columnTemplate = series.columns.template;
    ...
    function _catgraph(cat, curyr2) {
      $.ajax({
        url: "fetchdata/fetch_data.php",
        method: 'POST',
        data: { cat: cat, yr: curyr2, mode: 'netpie' },
        success: function(data) {
          var objcat = JSON.parse(data);
          _plotcatgrph(objcat);
          $('#cat_graph_modal').modal({'show': true,"backdrop": 'static'});
        }
      });
    }
 }
