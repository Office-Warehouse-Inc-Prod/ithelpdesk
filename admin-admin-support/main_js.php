 ...
 function get_card_data(y) {
  $.post('fetchdata/fetch_data.php', { yr: y, mode: 'yearch' }, function (data) {
    try {
      ...
      console.error("JSON Parse Error in get_card_data. Server returned HTML instead of JSON:", data);
      // Fallback display if data fails to load
      $('#count_total, #count_open, #count_owfa, #count_closed, #today_closed').html('Error');
    }
  });
}
