...
<div class="card-title">PENDING REPORTS: <span class="float-right" id="count_open" style="font-size: 32px;"></span></div>
<!-- <div class="row">
  <div class="col-md-3">
  <h6>Hardware</h6>
...
<div class="dashcard card text-white mb-4 bg-secondary" style="width: 18rem; height: 9rem; ">
<div class="card-body">

<div class="card-title">FOR PICK UP: <span class="float-right" id="count_forpickup" style="font-size: 32px;"></span></div>
</div>
<div class="card-footer d-flex align-items-center justify-content-between">

                       <a class=" text-white stretched-link" id="card_forpickup" href="#bottom" value="FOR PICKUP" ><span class="small text-white">Click here for more info.</span></a>
...
<div class="card-title">DELIVERED TO SUPPLIER: <span class="float-right" id="count_delsup" style="font-size: 32px;"></span></div>

</div>
<div class="card-footer d-flex align-items-center justify-content-between">

                       <a class=" text-white stretched-link" id="card_delsup" href="#bottom" value="DELIVERED TO SUPPLIER" ><span class="small text-white">Click here for more info.</span></a>
...
<div class="card-title">FOR DELIVERY TO STORE: <span class="float-right" id="count_fordev" style="font-size: 32px;"></span></div>

</div>
<div class="card-footer d-flex align-items-center justify-content-between">

                       <a class=" text-white stretched-link" id="card_delstore" href="#bottom" value="FOR DELIVERY TO STORE" ><span class="small text-white">Click here for more info.</span></a>
...
<option value="">Assign support...</option>  
     <?php
              $query="select * from it_tech WHERE itsup NOT IN ('4','7','8',12) AND deptsel = '4'";
              $run=$conn->prepare($query);
              $run->execute();
              $rs=$run->get_result();
              while ($res=$rs->fetch_assoc()) {
...
<div class="form-group col-4 col-md-4 col-lg-4 selected"  >
<label>STATUS</label>
<select class = "form-control form-control-sm" name= "status" id="status" required>
<option value=""> &larr; Status &rarr;</option>
<?php
      $query="select * from status WHERE ld_module_tag = 'Y'";
      $run=$conn->prepare($query);
      $run->execute();
      $rs=$run->get_result();
      while ($res=$rs->fetch_assoc()) {
...
<div class="form-group col-4 col-md-4 col-lg-4">
<label id="tripticket1lbl">TRIP TICKET# (FOR PICK UP)</label>
<input type="text" class="form form-control" name="tripticket1" id="tripticket1" placeholder="FOR PICK UP">
</div>

<div class="form-group col-4 col-md-4 col-lg-4">
<label id="tripticket2lbl">TRIP TICKET# (FOR DELIVERY TO STORE)</label>
<input type="text" class="form form-control" name="tripticket2" id="tripticket2" placeholder="FOR DELIVER TO STORE">
</div>
...
if (iN == "READY TO PICK UP") {
  $('#tripticket1lbl').fadeIn();
  $('#tripticket1').fadeIn();
  
}
else if (iN == "RETURN TO STORE") {
...
$("div.selected select").val(data['status']);
