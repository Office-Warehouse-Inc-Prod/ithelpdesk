...
";
	...
	`reports`.`itsup` AS `itsup`,
	`it_tech`.`it_desc` AS `it_desc`,
	`it_pr`.`it_desc` AS `it_sel`,
	concat_ws( '-', `reports`.`itsup`, `it_tech`.`it_desc` ) AS `tech_list`,
	concat_ws( '-', `reports`.`cat_id`, `categories`.`cat_desc` ) AS `cat_list`,
	`reports`.`cat_id` AS `cat_id`,
	`categories`.`cat_desc` AS `category`,
	concat_ws( '-', `subcat`.`sub_id`, `subcat`.`sub_cat` ) AS `sub_list`,
	`subcat`.`sub_id` AS `sub_id`,
	`subcat`.`sub_cat` AS `sub_category`,
	`reports`.`date_closed` AS `date_closed`,
	YEAR ( `reports`.`date_created` ) AS `years`,
	`reports`.`close_by` AS `close_by`,
	`tbl_clusers`.`it_desc` AS `clusers`,
	`reports`.`remarks` AS `remarks`,
	`reports`.`isp_id` AS `isp_id`,
	`tbl_isp`.`isp_shortDesc` AS `isp_shortDesc`,
	`reports`.`refNo` AS `refNo`,
	`reports`.`date_refNo` AS `date_refNo`,
	`reports`.`subject` AS `subject`,
	reports.tripticket1 AS tripticket1,
	reports.tripticket2 AS tripticket2,
	to_days( `reports`.`date_closed` ) - to_days( `reports`.`date_created` ) AS `tdc`,
	curdate() AS `crdt`,
	to_days(
	CURRENT_TIMESTAMP ()) - to_days( `reports`.`date_created` ) AS `dtdf`,
	`reports_msgcnt`.`msg_cnt` AS `msg_cnt` 
FROM
	((((((((
									`categories`
									JOIN `reports` ON ( `reports`.`cat_id` = `categories`.`cat_id` ))
								JOIN `subcat` ON ( `subcat`.`sub_id` = `reports`.`sub_id` ))
							JOIN `it_tech` ON ( `it_tech`.`itsup` = `reports`.`itsup` ))
						JOIN `it_pr` ON ( `it_pr`.`itsup` = `reports`.`itsup` ))
					JOIN `tbl_branch` ON ( `tbl_branch`.`str_num` = `reports`.`store` ))
				JOIN `tbl_clusers` ON ( `tbl_clusers`.`itsup` = `reports`.`close_by` ))
		LEFT JOIN `tbl_isp` ON ( `tbl_isp`.`isp_id` = `reports`.`isp_id` ))
	LEFT JOIN `reports_msgcnt` ON ( `reports`.`ticket_no` = `reports_msgcnt`.`ticket_no` )) 
	WHERE reports.deptsel = '4' AND
	 reports.sub_id NOT IN ('15','28','34','35') AND YEAR(reports.date_created) IN (".$_POST['yr'] .")
