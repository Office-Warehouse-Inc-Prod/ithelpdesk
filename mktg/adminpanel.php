...
<div class="card-title" style="font-size: 15px;">PROCESSING<span class="float-right" id="count_owfa" style="font-size: 32px;"></span></div>

</div>
<div class="card-footer d-flex align-items-center justify-content-between">
                      
                           <a class="text-white stretched-link" id="card_openwfaval" href="#bottom" value="ON PROCESS" ><span class="small text-white">Click here for more info.</span></a>
                          <div class="go-arrow">  </div>
                      </div>
...
<option value="">Assign support...</option>  
     <?php
              $query="select * from it_tech WHERE deptsel = '3' AND itsup NOT IN ('4','7','8',12)";
              $run=$conn->prepare($query);
              $run->execute();
              $rs=$run->get_result();
              while ($res=$rs->fetch_assoc()) {
...
$(row).find('td:eq(0)').css('color', '#FFC107');
$(row).find('td:eq(1)').css('color', '#FFC107');
$(row).find('td:eq(2)').css('color', '#FFC107');
$(row).find('td:eq(3)').css('color', '#FFC107');
$(row).find('td:eq(4)').css('color', '#FFC107');
$(row).find('td:eq(5)').css('color', '#FFC107');
$(row).find('td:eq(6)').css('color', '#FFC107');
$(row).find('td:eq(7)').css('color', '#FFC107');
$(row).find('td:eq(8)').css('color', '#FFC107');
$(row).find('td:eq(9)').css('color', '#FFC107');
$(row).find('td:eq(10)').css('color', '#FFC107');
$(row).find('td:eq(11)').css('color', '#FFC107');
$(row).find('td:eq(11)').html(' ');
$(row).find('td:eq(12)').css('color', '#FFC107');
}
...
option.text = $(this).parent().siblings(':nth-of-type(12)').html();
sst.add(option);   
