...
<!-- Form Validation and File Preview JavaScript -->
<script>
$(document).ready(function() {
    // File preview functionality for both forms
    function setupFilePreview(inputSelector, previewSelector) {
        $(inputSelector).on('change', function(e) {
            ...
            const allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'application/pdf', 'application/msword', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'];
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (!allowedTypes.includes(file.type)) {
                    alert(`File type not allowed: ${file.name}. Allowed: Images, PDF, Word, Excel`);
                    continue;
                }
                
                // Validate file size
                if (file.size > maxSize) {
                    alert(`File too large: ${file.name}. Max size: 2MB`);
                    continue;
                }
                ...
                const filePreview = $(`
                    <div class="file-preview-item" style="border: 1px solid #ddd; border-radius: 8px; padding: 10px; text-align: center; flex: 0 0 auto;">
                        <span class="badge badge-secondary">${fileSize}</span>
                        <p style="margin: 5px 0; font-size: 12px; word-break: break-word;">${fileName}</p>
                    </div>
                `);
    ...
    // Setup file previews
    setupFilePreview('#create_file-input', '#create_file_preview');
    setupFilePreview('#file-input', '#file_preview_container');
    
    // Form validation for create_report_form
    $('#create_report_form').on('submit', function(e) {
        const concern = $('#create_concern').val().trim();
        const subject = $('#create_subject').val();
        const store = $('#create_store').val();
        const dept = $('#create_deptsel').val();
        ...
        if (concern.length < 10) {
            e.preventDefault();
            alert('Concern/Message must be at least 10 characters');
            return false;
        }
        
        if (concern.length > 1000) {
            e.preventDefault();
            alert('Concern/Message cannot exceed 1000 characters');
            return false;
        }
    });
    
    // Form validation for report_form
    $('#report_form').on('submit', function(e) {
        const remarks = $('#addmsg').val().trim();
