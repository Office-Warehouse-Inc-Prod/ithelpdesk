...
pieSeries.slices.template.events.on("hit", function(ev){
  let srchval = ev.target.dataItem.category;
  var table = $("#report_data").DataTable();
  // alert(srchval);
  table.search(srchval).draw()
  $('html, body').animate({
        scrollTop: $("#report_data").offset().top
    }, 1000);
...
pieSeries.slices.template.adapter.add("fill", function(fill, target) {
  if (target.dataItem && (target.dataItem.category == 'OPEN')) {
    return am4core.color("#C02739");
  }
  if (target.dataItem && (target.dataItem.category == 'SCHEDULE FOR PULL OUT')) {
    return am4core.color("#1597BB");
  }
  if (target.dataItem && (target.dataItem.category == 'DIRECT PULL OUT')) {
    return am4core.color("#31363F");
  }
  ...
  if (target.dataItem && (target.dataItem.category == 'REPAIRED SAME UNIT AND SERIAL')) {
    return am4core.color("#662549");
  }
  if (target.dataItem && (target.dataItem.category == 'REPAIRED SAME UNIT DIFFERENT SERIAL')) {
    return am4core.color("#AE445A");
  }
  if (target.dataItem && (target.dataItem.category == 'REPLACE')) {
    return am4core.color("#451952");
  }
  if (target.dataItem && (target.dataItem.category == 'RTV')) {
    return am4core.color("#F39F5A");
  }
...
$(row).find('td:eq(1)').css('color', '#31363F');
...
$(row).find('td:eq(1)').css('color', '#662549');
...
$(row).find('td:eq(1)').css('color', '#AE445A');
...
$(row).find('td:eq(1)').css('color', '#451952');
...
$(row).find('td:eq(1)').css('color', '#F39F5A');
