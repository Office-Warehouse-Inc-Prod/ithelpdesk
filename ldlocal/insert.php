...
$ticknum = $res['ticket_no']+1;
  ...
  $statement = $connection->prepare("
   INSERT INTO reports (ticket_no, store, date_created, subject,  via, status, itsup, cat_id, sub_id, date_closed, close_by, remarks, isp_id) 
   VALUES (:ticket_no, :store, :date_created, :subject, :via, :status, :itsup, :cat_id, :sub_id, :date_closed, :close_by, :remarks, :isp_id)
  ");
  $dcval = $_POST["date_created"];
  $dclval = $_POST["date_closed"];
  $datetime = date_create($dcval)->format('Y-m-d H:i:s');
...
$data=   array(
    ...
    ':isp_id' => '0',
    ':refNo' => $_POST["refNo"],
    ':tripticket1' => $_POST["tripticket1"],
    ':tripticket2' => $_POST["tripticket2"],
    ':date_refNo' => date('Y-m-d H:i:s',strtotime($_POST["date_refNo"])),
    ':date_closed' => date('Y-m-d H:i:s',strtotime($_POST["date_closed"])),
    ':close_by' => $_POST["close_by"],
    ':remarks' => $_POST["remarks"],
    ':newserialno' => $_POST["newserialno"],
    ':newalu' => $_POST["newalu"],
    ':rtvno' => $_POST["rtvno"]
  ...
  $statement = $connection->prepare(
   "UPDATE reports
   SET ticket_no = :ticket_no, $brid date_created = :date_created,  via = :via, n_serial_no = :newserialno, n_alu = :newalu, rtv = :rtvno, 
                    status = :status, $itsup $cat_id $sub_id $ispid refNo = :refNo, tripticket1 = :tripticket1, tripticket2 = :tripticket2, date_refNo = :date_refNo, date_closed = :date_closed, $clby remarks = :remarks
   WHERE ticket_no = :ticket_no"
  );
...
if($_POST["operation"] == "Save and Reply2")
 { 
     $optbrval =$_POST["store"];
     $optval =$_POST["itsup"];
     $optcval =$_POST["cat"];
...
if (isset($_POST['operation3']) && $_POST['operation3'] == 'Update') {
  $data1 = $_POST['data1'];
  $IDx = $data1['itemId'];
  $n_alu_no = $data1['newAlu'];
  $n_description = $data1['newDescription'];
  $n_serial_no = $data1['newSerialNo'];
  $n_rtv = $data1['rtv'];
  $n_cm = $data1['cm'];
  $n_status = $data1['status'];

  $statement = $connection->prepare("UPDATE tbl_pditems SET n_alu_no = :n_alu_no, n_description = :n_description, n_serial_no = :n_serial_no, n_rtv = :n_rtv, n_cm = :n_cm, n_status = :n_status, item_status = :itemstat WHERE id = '$IDx'");

  $result = $statement->execute(array(
    ':n_alu_no' => $n_alu_no,
    ':n_description' => $n_description,
    ':n_serial_no' => $n_serial_no,
    ':n_rtv' => $n_rtv,
    ':n_cm' => $n_cm,
    ':n_status' => $n_status,
    ':itemstat' => 'Y',
  ...
  if (!$result) {
    echo "Error updating database: " . $statement->errorInfo()[2];
  } else {
    echo "UPDATED";
  }
}
