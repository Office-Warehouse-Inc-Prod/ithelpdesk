...
class dbconfig extends dbconn
{
	public function fetch_cards_result(){
		...
		AND reports.service_desc = 'LOCAL' AND reports.cwhtag = 'Y'
		";
	...
	public function overallpie_res(){
		...
		$query="
			...
			where `reports`.`sub_id` NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELPDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.deptsel = '4' AND reports.service_desc = 'LOCAL' AND reports.cwhtag = 'Y'
			GROUP BY `status`
...
// fix update cj for tech datatable
	public function bargrph_tech_res(){
		...
		$query="SELECT
				...
				OR reports.`status` = 'DIRECT PULL OUT' 
				OR reports.`status` = 'REPAIRED SAME UNIT AND SERIAL' 
				OR reports.`status` = 'REPAIRED SAME UNIT DIFFERENT SERIAL' 
				OR reports.`status` = 'REPLACE' 
				OR reports.`status` = 'RTV'
				OR reports.`status` = 'SUBJECT FOR CLOSING' THEN
					1 ELSE NULL 
				END) as openrep,
				...
				WHERE
				reports.sub_id NOT IN (15,28,34,35) AND reports.itsup NOT IN ('8') AND reports.deptsel = '4' AND reports.service_desc = 'LOCAL' AND reports.cwhtag = 'Y' AND
	...
	public function linegraph(){
		...
		reports
		WHERE
		year(date_created) BETWEEN '".$_POST['yr'] ."' AND '".$_POST['yr'] ."' and reports.sub_id NOT IN ('15','28','34','35') AND reports.deptsel = '4' AND reports.service_desc = 'LOCAL' AND reports.cwhtag = 'Y'
		GROUP BY
		DATEPART";	
		} else {
		$query="
	...
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( ".$_POST['yr'] ." ) AND deptsel = '4' AND reports.service_desc = 'LOCAL' AND reports.cwhtag = 'Y'
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement-> execute();
...
";
		...
		else  
		 {
			...
			select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (".$_POST['yr'] .") AND area_desc = '".$_POST['area_desc'] ."' AND deptsel = '4' AND reports.service_desc = 'LOCAL' AND reports.cwhtag = 'Y' group by reports.store, str_code, area_desc ORDER BY str_code ASC
	...
	$query="SELECT * from vw6pd WHERE vw6pd.deptsel = '4' AND vw6pd.service_desc = 'LOCAL' AND vw6pd.cwhtag = 'Y' AND
