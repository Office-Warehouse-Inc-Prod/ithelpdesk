...
class dbconfig extends dbconn
{
	public function fetch_cards_result(){
        ...
        FROM
        reports WHERE  `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.deptsel = '4'";
	...
	public function overallpie_res(){
		...
		WHERE  `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.deptsel = '4'
		GROUP BY `status`
...
// fix update cj for tech datatable
	public function bargrph_tech_res(){
		...
		ld_tech.f_name AS it_name,
		ld_tech.it_desc AS it_desc,
		ld_tech.cmp_role AS cmp_role,
	...
	FROM
		ld_tech
		LEFT JOIN reports ON reports.itsup = ld_tech.itsup
		LEFT JOIN users ON users.tech_id = ld_tech.itsup 
		AND reports.deptsel = users.deptsel 
	WHERE
	`status` NOT IN ( 'WAITING FOR IT HELDESK RESPONSE', 'NEW REPORT' ) 
		AND YEAR ( date_created ) IN (".$_POST['yr'] .") AND reports.deptsel = '4'
	GROUP BY
		reports.itsup 
	ORDER BY
		reports.itsup ASC
...
";
	...
	$query="
	Select * from vw7 WHERE vw7.deptsel = '4' AND
	vw7.sub_id NOT IN ('15','28','34','35') AND status <> 'WAITING FOR IT HELPDESK RESPONSE' AND YEAR(vw7.date_created) IN (".$_POST['yr'] .")";
...
public function newreporthist(){
	...
	$query="select reports.deptsel as deptsel, `reports`.`ticket_no` AS `ticket_no`,`reports`.`date_created` AS `date_created`,`reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_code`,`reports`.`concern` AS `concern`,`reports`.`service_desc` AS `service_desc`,`reports`.`subject` AS `subject`,`reports`.`status` AS `status`,`reports`.`userId` AS `userId`,`reports`.`via` AS `via`,`reports`.`itsup` AS `itsup`,`it_tech`.`it_desc` AS `it_desc`,`reports`.`cat_id` AS `cat_id`,`categories`.`cat_desc` AS `cat_desc`,concat_ws('-',`reports`.`cat_id`,`categories`.`cat_desc`) AS `cat_x`,`reports`.`sub_id` AS `sub_id`,`subcat`.`sub_cat` AS `sub_cat`,`reports`.`date_closed` AS `date_closed`,`reports`.`remarks` AS `remarks`,`reports_msgcnt`.`msg_cnt` AS `msg_cnt`,`reports_newmsg`.`nmsg_stat` AS `nmsg_stat`,`users`.`fname` AS `fname`,`users`.`lstname` AS `lstname`,concat_ws(' ',`users`.`fname`,`users`.`lstname`) AS `full_name` from (((((((`reports` join `tbl_branch` on(`tbl_branch`.`str_num` = `reports`.`store`)) left join `it_tech` on(`it_tech`.`itsup` = `reports`.`itsup`)) left join `categories` on(`categories`.`cat_id` = `reports`.`cat_id`)) left join `subcat` on(`subcat`.`sub_id` = `reports`.`sub_id`)) left join `reports_msgcnt` on(`reports_msgcnt`.`ticket_no` = `reports`.`ticket_no`)) left join `reports_newmsg` on(`reports_newmsg`.`ticket_no` = `reports`.`ticket_no`)) left join `users` on(`users`.`id` = `reports`.`userId`)) where `reports`.`status` = 'NEW REPORT' AND reports.deptsel = '4' order by `reports`.`date_created` desc ";
	$statement = $this->connection->prepare($query);
	$statement-> execute();
	$result = $statement->fetchAll();
	$data[] = array();
...
// fix update please main obj
	$query="
	Select * from vw_ld_ticket WHERE
	vw_ld_ticket.status <> 'WAITING FOR IT HELPDESK RESPONSE' AND itsup = ".$_POST['itVal']." AND years IN (".$_POST['yrsx1'].") ORDER BY `status` DESC";
	$statement = $this->connection->prepare($query);
	$statement-> execute();
	$result = $statement->fetchAll();
	$data[] = array();
