...
class dbconfig extends dbconn
{
	public function fetch_cards_result(){
		...
		CASE
				...
				OR reports.`status` = 'READY TO PICK UP'
				OR reports.`status` = 'CONFIRM PICK UP'
        ...
        FROM
        reports WHERE sub_id NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.deptsel = '4' AND reports.service_desc = 'REPAIR IMPORT' ";
	...
	public function overallpie_res(){
		...
		$query="
			...
			where `reports`.`sub_id` NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELPDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.deptsel = '4' AND reports.service_desc = 'REPAIR IMPORT'
			GROUP BY `status`
...
// fix update cj for tech datatable
	public function bargrph_tech_res(){
		...
		$query="SELECT
				...
				WHERE
				reports.sub_id NOT IN (15,28,34,35) AND reports.itsup NOT IN ('8') AND reports.deptsel = '4' AND reports.service_desc = 'REPAIR IMPORT' AND
	...
	public function linegraph(){
		...
		reports
		WHERE
		year(date_created) BETWEEN '".$_POST['yr'] ."' AND '".$_POST['yr'] ."' and reports.sub_id NOT IN ('15','28','34','35') AND reports.deptsel = '4' AND reports.service_desc = 'REPAIR IMPORT'
		GROUP BY
		DATEPART";	
		} else {
		$query="
	...
	public function pie(){
		...
		FROM vwpd 
		WHERE deptsel = '4' AND service_desc = 'REPAIR IMPORT' AND date_created IN (".$_POST['yr'] .")
	...
	public function subs($id){
		...
		$query= "SELECT sub_cat, count(*) as sctn, date_created FROM vwpd WHERE cat_id='".$id."' AND deptsel = '4' AND service_desc = 'REPAIR IMPORT' AND date_created IN (".$_POST['yr'] .")  GROUP BY sub_cat ORDER BY cat_desc ASC";
	...
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( ".$_POST['yr'] ." ) AND deptsel = '4' AND reports.service_desc = 'REPAIR IMPORT'
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement-> execute();
...
";
		...
		else  
		 {
			...
			select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (".$_POST['yr'] .") AND area_desc = '".$_POST['area_desc'] ."' AND deptsel = '4' AND reports.service_desc = 'REPAIR IMPORT' group by reports.store, str_code, area_desc ORDER BY str_code ASC
	...
	$query="SELECT * from vw6pd WHERE vw6pd.deptsel = '4' AND vw6pd.service_desc = 'REPAIR IMPORT' AND
...
WHERE
	...
	AND reports.service_desc = 'REPAIR IMPORT' 
ORDER BY
