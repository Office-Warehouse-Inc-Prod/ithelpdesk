...
public function deptthist() {
	...
	public function linegraph(){
		...
		reports
		WHERE
		year(date_created) BETWEEN '2019' AND '2022' and reports.sub_id NOT IN ('15','28','34','35') AND reports.f_deptsel = '6' 
		GROUP BY
		DATEPART";
		}
...
";
		...
		else  
		 {
			...
			select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (".$_POST['yr'] .") AND area_desc = '".$_POST['area_desc'] ."' AND f_deptsel = '6' group by reports.store, str_code, area_desc ORDER BY str_code ASC
...
//exclude from deptsel migration to f_deptsel since vw6 f_deptsel  AS "dept_sel"
	...
	$query="SELECT DISTINCT vw6.*
		...
		AND YEAR(vw6.date_created) IN (" . $_POST['yr'] . ") ORDER BY vw6.date_created DESC";
	...
	public function admin_data_table_transfer()
{
    ...
    WHERE vw_transfer.deptsel = '6' 
    AND vw_transfer.f_deptsel NOT IN ('6') 
