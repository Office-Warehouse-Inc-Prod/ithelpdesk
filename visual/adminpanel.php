...
<div id="welcomeModal" class="modal-overlay" style="display: none;">
    <div class="modal-content">
        <h3>Visual HelpDesk Efficiency & Performance Report</h3>
                  ...
                  <select class="form-control form-control-sm" name="cat" id="cat" required>
                    ...
                    <?php
                      // $query="select * from category WHERE deptsel = '1'";
                          $query="select * from categories WHERE deptsel = '6' AND old_tag IS NULL";
                      $run=$conn->prepare($query);
                      $run->execute();
                      $rs=$run->get_result();
                      while ($res=$rs->fetch_assoc()) {
...
if (!empty($ticket_id)) {
    ...
    $fetch_query = "SELECT status FROM reports WHERE id = ?"; 
    $fetch_stmt = $conn->prepare($fetch_query);
    $fetch_stmt->bind_param("i", $ticket_id); 
    $fetch_stmt->execute();
    $fetch_result = $fetch_stmt->get_result();
    
    if ($row = $fetch_result->fetch_assoc()) {
        $current_ticket_status = $row['status'];
    }
}
?>
<div class="form-group col-12 col-md-4">
  ...
  <select class="form-control form-control-sm" name="status" id="status" required>
    ...
    <?php
      ...
      $run->execute();
      $rs=$run->get_result();
      $restricted_statuses = ['PENDING', 'CLOSED', 'SUBJECT FOR CLOSING'];

      while ($res=$rs->fetch_assoc()) {
          $option_status = $res['stat_desc'];
          if ($option_status === 'ON PROCESS' && in_array($current_ticket_status, $restricted_statuses)) {
              continue; 
          }
          $is_selected = ($option_status === $current_ticket_status) ? 'selected' : '';
    ?>
      <option value="<?= htmlspecialchars($option_status) ?>" <?= $is_selected ?>>
        <?= htmlspecialchars($option_status) ?>
      </option>
    <?php } ?>
  </select>
</div>
