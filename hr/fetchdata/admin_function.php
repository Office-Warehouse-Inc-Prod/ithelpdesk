...
class dbconfig extends dbconn
{
	public function fetch_cards_result(){
        ...
        FROM
        reports WHERE sub_id NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.f_deptsel = '11'";
	...
	public function overallpie_res(){
		...
		$query="
			...
			where `reports`.`sub_id` NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELPDESK RESPONSE','NEW REPORT','ASSIGNED') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.f_deptsel = '11'
			GROUP BY `status`
...
// fix update cj for tech datatable
	public function bargrph_tech_res(){
		...
		$query="SELECT
				...
				WHERE
				reports.sub_id NOT IN (15,28,34,35) AND reports.itsup NOT IN ('8') AND reports.f_deptsel = '11' and
	...
	public function linegraph(){
		...
		reports
		WHERE
		year(date_created) BETWEEN '".$_POST['yr'] ."' AND '".$_POST['yr'] ."' and reports.sub_id NOT IN ('15','28','34','35') AND reports.f_deptsel = '11'
		GROUP BY
		DATEPART";	
		} else {
		$query="
...
// exclude from changing deptsel to f_deptsel
		...
		FROM vwp 
		WHERE deptsel = '11' AND date_created IN (".$_POST['yr'] .")  AND cat_desc <> 'GENERAL'
...
// exclude from changing deptsel to f_deptsel
	...
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( ".$_POST['yr'] ." ) AND f_deptsel = '11'
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement-> execute();
