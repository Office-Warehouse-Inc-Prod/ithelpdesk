...
class dbconfig extends dbconn
{
	...
	public function fetch_cards_result(){
        ...
        FROM
        reports WHERE sub_id NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND cat_id IN ('42','43','44','45','46') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.f_deptsel = '11'";
	...
	public function overallpie_res(){
		...
		$query="
			...
			where `reports`.`sub_id` NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELPDESK RESPONSE','NEW REPORT','ASSIGNED') AND cat_id IN ('42','43','44','45','46') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.f_deptsel = '11'
			GROUP BY `status`
...
// fix update cj for tech datatable
	public function bargrph_tech_res(){
		...
		$query="SELECT
				...
				WHERE
				reports.sub_id NOT IN (15,28,34,35) AND reports.itsup NOT IN ('8') AND reports.f_deptsel = '11' AND cat_id IN ('42','43','44','45','46')  AND
	...
	public function linegraph(){
		...
		reports
		WHERE
		year(date_created) BETWEEN '".$_POST['yr'] ."' AND '".$_POST['yr'] ."' and reports.sub_id NOT IN ('15','28','34','35') AND cat_id IN ('42','43','44','45','46') AND reports.f_deptsel = '11'
		GROUP BY
		DATEPART";	
		} else {
		$query="
		...
		reports
		WHERE
		year(date_created) BETWEEN '2019' AND '2025	' and reports.sub_id NOT IN ('15','28','34','35') AND cat_id IN ('42','43','44','45','46') AND reports.f_deptsel = '11' 
		GROUP BY
		DATEPART";
		}
...
// exclude from changing deptsel to f_deptsel
		...
		FROM vwp 
		WHERE deptsel = '11' AND cat_id IN ('42','43','44','45','46') AND date_created IN (".$_POST['yr'] .")  AND cat_desc <> 'GENERAL'
...
// exclude from changing deptsel to f_deptsel
		$query= "SELECT sub_cat, count(*) as sctn, date_created FROM vwp WHERE cat_id='".$id."' AND deptsel = '11'  AND date_created IN (".$_POST['yr'] .")  GROUP BY sub_cat ORDER BY cat_desc ASC";
	...
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( ".$_POST['yr'] ." ) AND f_deptsel = '11' AND cat_id IN ('42','43','44','45','46') 
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement-> execute();
...
";
		...
		else  
		 {
			...
			select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (".$_POST['yr'] .") AND area_desc = '".$_POST['area_desc'] ."' AND f_deptsel = '11' group by reports.store, str_code, area_desc ORDER BY str_code ASC
...
//exclude from deptsel migration to f_deptsel since vw6 f_deptsel  AS "dept_sel"
	...
	// $query="
	// Select * from vw6 WHERE vw6.deptsel = '11' AND
	// vw6.sub_id NOT IN ('15','28','34','35') AND status <> 'NEW REPORT' AND YEAR(vw6.date_created) IN (".$_POST['yr'] .")";
		...
		WHERE (
			(vw6.deptsel = '11' AND vw6.cat_id IN ('42','43','44','45','46') AND vw6.status NOT IN ('NEW REPORT', 'Assigned', 'ASSIGNED'))
			OR 
			(users.deptsel = '11' AND vw6.status IN ('NEW REPORT'))
		)
	...
	public function admin_data_table_transfer()
{
    ...
    WHERE vw_transfer.deptsel = '11' 
    AND vw_transfer.f_deptsel NOT IN ('11') 
...
WHERE
	...
	public function netpie(){
		...
		FROM vwp 
		WHERE deptsel = '11' AND cat_id ='3' AND date_created IN (".$_POST['yr'] .")
		GROUP BY cat_id ORDER BY cat_desc ASC";
		$statement = $this->connection->prepare($query);
		$statement-> execute();
		$result = $statement->fetchAll();
	...
	public function overallnet_res(){
		...
		$query="SELECT
			...
			reports
			LEFT JOIN tbl_status ON reports.`status` = tbl_status.stat_desc
			where `reports`.`sub_id` NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELPDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.deptsel = '11' AND cat_id = '3'
			GROUP BY `status`
			ORDER BY stat_id ASC
		";
	...
	FROM
		...
		JOIN `tbl_area` ON ( `tbl_area`.`area_num` = `tbl_branch`.`area_num` )) 
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( ".$_POST['yr'] ." ) AND deptsel = '11' AND cat_id = '3'
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement-> execute();
...
";
		 ...
		 {
			$query="select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (".$_POST['yr'] .") AND area_desc = '".$_POST['area_desc'] ."' AND deptsel = '11' AND cat_id = '3' group by reports.store, str_code, area_desc ORDER BY str_code ASC
	...
	public function admin_data_table_resnet(){
		...
		$query="
		Select * from vw6 WHERE vw6.deptsel = '11' AND cat_id = '3' AND
