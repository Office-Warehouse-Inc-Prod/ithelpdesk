...
class dbconfig extends dbconn
{
	...
	public function fetch_cards_result()
	{
        ...
        FROM
        reports WHERE sub_id NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (" . $_POST['yr'] . ") AND reports.f_deptsel = '4'";
	...
	public function overallpie_res()
	{
		...
		$query = "
			...
			where `reports`.`sub_id` NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELPDESK RESPONSE','NEW REPORT','ASSIGNED') AND YEAR(date_created) IN (" . $_POST['yr'] . ") AND reports.f_deptsel = '4'
			GROUP BY `status`
	...
	public function bargrph_tech_res()
	{
		...
		$query = "SELECT
				...
				WHERE
				reports.sub_id NOT IN (15,28,34,35) AND reports.itsup IN (1,2,5,7,10,11,14,44,45,47,77) AND reports.f_deptsel = '4' AND
	...
	public function linegraph()
	{
		...
		reports
		WHERE
		year(date_created) BETWEEN '" . $_POST['yr'] . "' AND '" . $_POST['yr'] . "' and reports.sub_id NOT IN ('15','28','34','35') AND reports.f_deptsel = '4'
		GROUP BY
		DATEPART";
		} else {
			$query = "
		...
		reports
		WHERE
		year(date_created) BETWEEN '2019' AND '2025' and reports.sub_id NOT IN ('15','28','34','35') AND reports.f_deptsel = '4'
		GROUP BY
		DATEPART";
		}
	...
	public function pie()
	{
		...
		FROM vwp 
		WHERE deptsel = '4' AND date_created IN (" . $_POST['yr'] . ")  AND cat_desc <> 'GENERAL'
	...
	{
		// exclude from changing deptsel to f_deptsel
		$query = "SELECT sub_cat, count(*) as sctn, date_created FROM vwp WHERE cat_id='" . $id . "' AND deptsel = '4'  AND date_created IN (" . $_POST['yr'] . ")  GROUP BY sub_cat ORDER BY cat_desc ASC";
	...
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( " . $_POST['yr'] . " ) AND f_deptsel = '4'
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement->execute();
...
";
			...
			select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (" . $_POST['yr'] . ") AND area_desc = '" . $_POST['area_desc'] . "' AND f_deptsel = '4' group by reports.store, str_code, area_desc ORDER BY str_code ASC
	...
	public function admin_data_table_res()
	{
		...
		WHERE (
			(vw6.deptsel = '4' AND vw6.status NOT IN ('NEW REPORT', 'Assigned', 'ASSIGNED'))
			OR 
			(users.deptsel = '4' AND vw6.status NOT IN ('NEW REPORT'))
		)
...
";
	...
	public function dtbl_itsup()
	{
		...
		$query = "
	Select * from vw6 WHERE
	vw6.sub_id NOT IN ('15','28','34','35') AND status <> 'NEW REPORT' AND itsup = " . $_POST['itVal'] . " AND years IN (" . $_POST['yrsx1'] . ") AND deptsel = 4 ORDER BY `status` DESC";
		$statement = $this->connection->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
		$data[] = array();
...
WHERE
	...
	public function netpie()
	{
		...
		FROM vwp 
		WHERE deptsel = '4' AND cat_id ='3' AND date_created IN (" . $_POST['yr'] . ")
		GROUP BY cat_id ORDER BY cat_desc ASC";
		$statement = $this->connection->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
	...
	public function overallnet_res()
	{
		...
		$query = "SELECT
			...
			reports
			LEFT JOIN tbl_status ON reports.`status` = tbl_status.stat_desc
			where `reports`.`sub_id` NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELPDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (" . $_POST['yr'] . ") AND reports.deptsel = '4' AND cat_id = '3'
			GROUP BY `status`
			ORDER BY stat_id ASC
		";
	...
	FROM
		...
		JOIN `tbl_area` ON ( `tbl_area`.`area_num` = `tbl_branch`.`area_num` )) 
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( " . $_POST['yr'] . " ) AND deptsel = '4' AND cat_id = '3'
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement->execute();
...
";
			...
			$query = "select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (" . $_POST['yr'] . ") AND area_desc = '" . $_POST['area_desc'] . "' AND deptsel = '4' AND cat_id = '3' group by reports.store, str_code, area_desc ORDER BY str_code ASC
...
public function admin_data_table_transfer()
	{
		...
		WHERE vw_transfer.deptsel = '4' AND vw_transfer.f_deptsel NOT IN ('4') AND vw_transfer.status NOT IN ('ATTENDED WITH FIX ASSET','NEW REPORT', 'Assigned', 'ASSIGNED') 
	...
	public function admin_data_table_resnet()
	{
		...
		$query = "
		Select * from vw6 WHERE vw6.deptsel = '4' AND cat_id = '3' AND
