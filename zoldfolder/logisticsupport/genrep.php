...
$('#search').click(function(){
  ...
  if(start_date !='' && YEARS !='')
  {
    let x = "start_date="+start_date+"&end_date="+YEARS;
      $.ajax({
    url:"fetchdata/fetch_genpie.php",
    method:'POST',
    data:x,
    datatype:'JSON',
...
pieSeries.labels.template.text = "{stat_name}: {value.value}";
pieSeries.slices.template.tooltipText = "{stat_name}: {value.value}";
pieSeries.slices.template.tooltipPosition = "pointer";
  ...
  for (let i = 0; i < types.length; i++) {
    if (i == selected) {
      for (let x = 0; x < types[i].subs.length; x++) {
        chartData.push({
          stat_name: types[i].subs[x].stat_name,
          points: types[i].subs[x].points,
          pulled: true
        });
      }
    } else {
      chartData.push({
        stat_name: types[i].stat_name,
        points: types[i].points,
        id: i
      });
    }
  }
