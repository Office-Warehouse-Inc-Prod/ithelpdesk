...
class dbconfig extends dbconn
{
	public function fetch_cards_result(){
		...
		CASE
				...
				OR reports.`status` = 'ALREADY PICK UP' 
				OR reports.`status` = 'RETURN TO STORE' 
				OR reports.`status` = 'APRROVED' 
				OR reports.`status` = 'EVLAUATE'
				OR reports.`status` = 'REPAIRED'
				OR reports.`status` = 'REQUEST FOR I.R.'
		...
		COUNT(CASE WHEN reports.`status` = 'WAREHOUSE PULL OUT' then 1 else NULL end ) as t_sched,
        COUNT(CASE WHEN reports.`status` = 'OKAY FOR PULL OUT' then 1 else NULL end) as t_okay,
        COUNT(CASE WHEN reports.`status` = 'READY FOR PULL OUT' then 1 else NULL end) as t_pickup,
		COUNT(CASE WHEN reports.`status` = 'APPROVED' then 1 else NULL end) as t_approved,
		COUNT(CASE WHEN reports.`status` = 'EVALUATE' then 1 else NULL end) as t_evaluate,
		...
		COUNT(CASE WHEN reports.`status` = 'CONFIRM PULL OUT' then 1 else NULL end) as t_confirm,
		COUNT(CASE WHEN reports.`status` = 'ITEM RECEIVED' then 1 else NULL end) as t_received,
		COUNT(CASE WHEN reports.`status` = 'LIST FOR DISPOSAL' then 1 else NULL end) as t_list
        FROM
        reports WHERE sub_id NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.deptsel = '4' AND reports.service_desc = 'IMPORT' ";
	...
	public function overallpie_res(){
		...
		$query="
			...
			where `reports`.`sub_id` NOT IN ('15','28','34','35') AND `status` NOT IN ('WAITING FOR IT HELPDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .") AND reports.deptsel = '4' AND reports.service_desc = 'IMPORT'
			GROUP BY `status`
...
// fix update cj for tech datatable
	public function bargrph_tech_res(){
		...
		$query="SELECT
				...
				Count(CASE
				WHEN reports.`status` = 'OPEN' 
				OR reports.`status` = 'SCHEDULE FOR PULL OUT' 
				OR reports.`status` = 'READY TO PICK UP' 
				OR reports.`status` = 'ALREADY PICK UP' 
				OR reports.`status` = 'RETURN TO STORE' 
				OR reports.`status` = 'APRROVED' 
				OR reports.`status` = 'EVLAUATE'
				...
				WHERE
				reports.sub_id NOT IN (15,28,34,35) AND reports.itsup NOT IN ('8') AND reports.deptsel = '4' AND reports.service_desc = 'IMPORT' AND
	...
	public function linegraph(){
		...
		reports
		WHERE
		year(date_created) BETWEEN '".$_POST['yr'] ."' AND '".$_POST['yr'] ."' and reports.sub_id NOT IN ('15','28','34','35') AND reports.deptsel = '4' AND reports.service_desc = 'IMPORT'
		GROUP BY
		DATEPART";	
		} else {
		$query="
	...
	public function pie(){
		...
		FROM vwpd 
		WHERE deptsel = '4' AND service_desc = 'IMPORT' AND date_created IN (".$_POST['yr'] .")
	...
	public function subs($id){
		...
		$query= "SELECT sub_cat, count(*) as sctn, date_created FROM vwpd WHERE cat_id='".$id."' AND deptsel = '4' AND service_desc = 'IMPORT' AND date_created IN (".$_POST['yr'] .")  GROUP BY sub_cat ORDER BY cat_desc ASC";
	...
	WHERE
		YEAR ( `reports`.`date_created` )  IN ( ".$_POST['yr'] ." ) AND deptsel = '4' AND reports.service_desc = 'IMPORT'
	GROUP BY
		`tbl_branch`.`area_num`";
		$statement = $this->connection->prepare($query);
		$statement-> execute();
...
";
		...
		else  
		 {
			...
			select `reports`.`store` AS `store`,`tbl_branch`.`str_code` AS `str_dept`,`tbl_branch`.`area_num` AS `area_num`,`tbl_area`.`area_desc` AS `area_desc`,year(`reports`.`date_created`) AS `dc`,count(`reports`.`date_created`) AS `cnt_ttl` from ((`reports` join `tbl_branch` on(`reports`.`store` = `tbl_branch`.`str_num`)) join `tbl_area` on(`tbl_area`.`area_num` = `tbl_branch`.`area_num`)) WHERE YEAR(`reports`.`date_created`) IN (".$_POST['yr'] .") AND area_desc = '".$_POST['area_desc'] ."' AND deptsel = '4' AND reports.service_desc = 'IMPORT' group by reports.store, str_code, area_desc ORDER BY str_code ASC
	...
	$query="SELECT * from vw6pd WHERE vw6pd.deptsel = '4' AND vw6pd.service_desc = 'IMPORT' AND
	...
	// $fetchdata[] = array();
		...
		foreach($result as $row)
				...
				$fetchdata[] = array(
					...
					'rtv' => $row['rtv'],
					'tripticket1' => $row['tripticket1'],
					'tripticket2' => $row['tripticket2'],
...
WHERE
	...
	AND reports.service_desc = 'IMPORT' 
ORDER BY
...
WHERE
	...
	reports.deptsel = 4  AND reports.service_desc = 'IMPORT'
ORDER BY
	notif_date ASC ";
	$statement = $this->connection->prepare($query);
	$statement-> execute();
