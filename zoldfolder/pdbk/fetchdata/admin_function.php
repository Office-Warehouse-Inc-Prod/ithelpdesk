...
class dbconfig extends dbconn
{
	public function fetch_cards_result(){
        ...
        FROM
        reports WHERE  `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .")";
	...
	public function overallpie_res(){
		...
		WHERE  `status` NOT IN ('WAITING FOR IT HELDESK RESPONSE','NEW REPORT') AND YEAR(date_created) IN (".$_POST['yr'] .")
		GROUP BY `status`
...
// fix update cj for tech datatable
	...
	public function subs($id){
		...
		$query= "SELECT sub_cat, count(*) as sctn, date_created FROM vw_ld_categories WHERE cat_id='".$id."' AND date_created IN (".$_POST['yr'] .") GROUP BY sub_cat ORDER BY cat_desc ASC";
...
";
	...
	vw_ld_ticket.status <> 'WAITING FOR IT HELPDESK RESPONSE' AND YEAR(vw_ld_ticket.date_created) IN (".$_POST['yr'] .")";
