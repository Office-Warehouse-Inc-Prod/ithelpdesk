    ...
    <div class="modal-body">
      ...
      <div class="form-group col-md-8">
        ...
        <option value=""style="color:red;">&larr;ASSIGN SUPPORT&larr;</option>  
          <?php
            $query="select * from it_tech WHERE itsup NOT IN ('4','7','8','12','14') AND deptsel = '13'";
            $run=$con1->prepare($query);
            $run->execute();
            $rs=$run->get_result();
            while ($res=$rs->fetch_assoc()) {
      ...
      <div class="form-group col-md-6">
        ...
        <?php
          $query="select * from categories WHERE deptsel = '13' AND (old_tag IS NULL OR old_tag <> 'Y') ORDER BY order_id ASC";
          $run=$con1->prepare($query);
          $run->execute();
          $rs=$run->get_result();
          while ($res=$rs->fetch_assoc()) {
